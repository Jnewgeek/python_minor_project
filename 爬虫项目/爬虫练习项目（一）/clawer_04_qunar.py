...
# 获取景点数据
def get_data(content,table=None):
...
if __name__=="__main__":
    ...
    # 清洗数据
    for i in ["评分","排名","攻略提到数量","点评数量"]:
        data[i].fillna(0,inplace=True)
        data[i]=data[i].astype(int)
        
    for i in ["驴友到达比例","经度","纬度"]:
        data[i].fillna(0,inplace=True)
        data[i]=data[i].astype(float)
        
    data["驴友到达比例"]=data["驴友到达比例"]/100.0
    
    data.to_excel("案例4_去哪儿网%s景点数据%d条.xlsx"%(cityname,len(data)),index=False)
